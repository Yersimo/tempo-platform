                ...
                {recommended.length > 0 && (
                  <div>
                    <h3 className="text-sm font-semibold text-t1 mb-3 flex items-center gap-2">
                      <Zap size={16} className="text-tempo-700" />
                      Recommended for You
                    </h3>
                      ...
                      <div className="flex items-start justify-between mb-4">
                        <div>
                          <h3 className="text-sm font-semibold text-t1 flex items-center gap-2">
                            <GitBranch size={16} className="text-tempo-700" />
                            {path.name}
                          </h3>
                        ...
                        {steps.map((step, i) => (
                          <div key={i} className="flex items-start flex-shrink-0">
                            <div className="flex flex-col items-center">
                              <div className={`w-10 h-10 rounded-full flex items-center justify-center text-xs font-bold ${i === 0 ? 'bg-tempo-700/15 text-tempo-700 border-2 border-tempo-700/30' : 'bg-canvas text-t2 border border-main'}`}>
                                {step.level}
                              </div>
                ...
                <Card className="p-5">
                  <CardHeader className="px-0 pt-0">
                    <CardTitle className="text-sm flex items-center gap-2">
                      <Compass size={16} className="text-tempo-700" />
                      Career Interests
                    </CardTitle>
                  </CardHeader>
                  {myCareerInterest ? (
                  ...
                  <div className="space-y-3">
                    {recommended.slice(0, 5).map(({ gig, matchScore }) => (
                      <div key={gig.id} className="flex items-center gap-3 p-3 rounded-lg bg-canvas border border-main">
                        <div className="flex-shrink-0 w-8 h-8 rounded-lg bg-tempo-700/10 flex items-center justify-center text-tempo-700">
                        ...
                        <div className="flex items-center gap-2">
                          <div className="text-right">
                            <p className="text-xs font-bold text-tempo-700">{matchScore}%</p>
                      ...
                      return (
                        <div key={type.value} className="flex items-center gap-3">
                          ...
                          <div className="flex-1 bg-canvas rounded-full h-2 border border-main overflow-hidden">
                            <div className="h-full bg-tempo-700 rounded-full transition-all" style={{ width: `${pct}%` }} />
                          </div>
              ...
              {(() => {
                ...
                return (
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm font-bold text-tempo-700">{match.score}%</span>
                  </div>
                )
              })()}
            </div>
...
function GigCard({
  ...
  return (
    <Card className="p-4 hover:border-tempo-700/30 transition-colors group cursor-pointer" onClick={onView}>
      <div className="flex items-start justify-between mb-2">
        <div className="flex items-center gap-2">
          ...
          <div>
            <h3 className="text-sm font-semibold text-t1 group-hover:text-tempo-700 transition-colors">{gig.title}</h3>
            <p className="text-[10px] text-t3">{gigType?.label}</p>
          </div>
        </div>
        {matchScore != null && matchScore > 0 && (
          <div className="flex items-center gap-1 bg-tempo-700/10 px-2 py-1 rounded-full">
            <Star size={10} className="text-tempo-700 fill-tempo-700" />
            <span className="text-[10px] font-bold text-tempo-700">{matchScore}%</span>
          </div>
        )}
      </div>
  ...
  return (
    <Card className="p-6 max-w-2xl">
      <h3 className="text-sm font-semibold text-t1 mb-4 flex items-center gap-2">
        <Plus size={16} className="text-tempo-700" />
              ...
              return (
                <button
                  ...
                  className={`text-[10px] px-2 py-1 rounded-full border transition-colors ${
                    isSelected ? 'bg-tempo-700/15 border-tempo-700/30 text-tempo-700' : 'bg-canvas border-main text-t3 hover:border-tempo-700/20'
                  }`}
                >
                  {sk.name}
                </button>
