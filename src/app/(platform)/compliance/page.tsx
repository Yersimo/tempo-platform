...
const SEVERITY_COLORS: Record<string, string> = {
  ...
  high: 'bg-tempo-700/10 text-tempo-400 border-tempo-700/20',
      ...
      {activeTab === 'documents' && (
        <div className="space-y-4">
          ...
          {expiringDocs.length > 0 && (
            <div className="rounded-xl border border-tempo-700/20 bg-tempo-700/5 p-3">
              <div className="flex items-center gap-2">
                <Clock size={16} className="text-tempo-400" />
                <span className="text-xs font-medium text-tempo-400">
                  ...
                  {complianceDocuments.slice((docPage - 1) * ITEMS_PER_PAGE, docPage * ITEMS_PER_PAGE).map(doc => {
                    ...
                    return (
                      <tr key={doc.id} className="hover:bg-white/[0.02] transition-colors">
                        ...
                        <td className="px-4 py-3">
                          {doc.expires_at ? (
                            <span className={isExpiringSoon ? 'text-tempo-400 font-medium' : 'text-t2'}>{doc.expires_at}</span>
                          ) : (
                  ...
                  <div className="flex items-start gap-3">
                    <div className="flex-shrink-0 mt-0.5">
                      {alert.severity === 'critical' ? <XCircle size={16} className="text-red-400" /> :
                       alert.severity === 'high' ? <AlertTriangle size={16} className="text-tempo-400" /> :
