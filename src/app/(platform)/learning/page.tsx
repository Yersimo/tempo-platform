...
export default function LearningPage() {
  ...
  function handleCourseCompleted(courseId: string) {
    ...
    addCertificateTemplate({
      ...
      accentColor: (template as any)?.accentColor || 'var(--color-tempo-600)',
  ...
  function exportScormPackage() {
    ...
    const blockHtml = blocks.map(block => {
      ...
      switch (block.type) {
        case 'heading': return `<h2 style="font-size:1.5rem;font-weight:700;border-bottom:2px solid #285B7A;padding-bottom:0.5rem;margin:2rem 0 1rem">${block.content || block.title}</h2>`
        ...
        case 'button': return `<div style="text-align:center;margin:2rem 0"><a href="${parsed.url || '#'}" style="display:inline-block;padding:0.75rem 2rem;background:#285B7A;color:white;border-radius:8px;text-decoration:none;font-weight:600">${parsed.label || 'Continue'}</a></div>`
...
.header{background:linear-gradient(135deg,#285B7A,#00352C);color:white;padding:3rem 2rem;border-radius:16px;margin-bottom:2rem;text-align:center}
...
.progress-fill{height:100%;background:#285B7A;border-radius:3px;transition:width 0.3s;width:0%}
...
.nav button.primary{background:#285B7A;color:white;border:none}
...
<\/script>
  ...
  const providerColor = (p: string) => {
    switch (p) {
      ...
      case 'opensesame': return 'bg-tempo-700/10 text-tempo-400 border-tempo-700/20'
  ...
  const allBadgeTypes = [
    ...
    { type: 'streak_7', name: '7-Day Streak', icon: 'flame', description: 'Maintain a 7-day learning streak', color: 'text-tempo-400' },
  ...
  function exportTranscriptPDF() {
    ...
    if (printWindow) {
      ...
      printWindow.document.write(`<html><head><title>Learning Transcript - ${empName}</title><style>body{font-family:-apple-system,sans-serif;padding:40px;color:#fff;background:#0a0a0a}h1{color:#285B7A;font-size:20px}h2{font-size:16px;color:#ccc}table{width:100%;border-collapse:collapse;margin-top:20px}th{text-align:left;padding:8px;border-bottom:2px solid #285B7A;color:#285B7A;font-size:13px}td{font-size:13px;color:#ccc}.summary{display:flex;gap:30px;margin:20px 0}.stat{text-align:center}.stat-val{font-size:24px;font-weight:bold;color:#285B7A}.stat-label{font-size:11px;color:#999}</style></head><body><h1>Learning Transcript</h1><h2>${empName}</h2><div class="summary"><div class="stat"><div class="stat-val">${transcriptSummary.totalCourses}</div><div class="stat-label">Courses</div></div><div class="stat"><div class="stat-val">${transcriptSummary.totalHours}h</div><div class="stat-label">Total Hours</div></div><div class="stat"><div class="stat-val">${transcriptSummary.certificates}</div><div class="stat-label">Certificates</div></div>${transcriptSummary.avgScore ? `<div class="stat"><div class="stat-val">${transcriptSummary.avgScore}%</div><div class="stat-label">Avg Score</div></div>` : ''}</div><table><thead><tr><th>Course</th><th>Category</th><th>Completed</th><th>Score</th><th>Hours</th></tr></thead><tbody>${rows}</tbody></table><p style="margin-top:30px;font-size:11px;color:#666">Generated by Tempo Platform on ${new Date().toLocaleDateString()}</p></body></html>`)
              ...
              {!selectedBuilderCourse ? (
                ...
                <div className="space-y-0">
                  ...
                  {(() => {
                    ...
                    const coverColors: Record<string, string> = { tempo: 'from-tempo-600 to-tempo-700', blue: 'from-blue-600 to-cyan-500', green: 'from-green-600 to-emerald-500', purple: 'from-purple-600 to-pink-500', red: 'from-red-600 to-rose-500' }
                    return (
                        ...
                        >
                          ...
                          <div className="p-4">
                            ...
                            {block.type === 'infographic' && (
                              <div className="bg-gradient-to-br from-tempo-500/5 to-tempo-700/5 rounded-lg p-4 border border-tempo-500/10">
                                <div className="flex items-center gap-2 mb-3">
      ...
      <Modal open={showNewCourseFlow} onClose={() => setShowNewCourseFlow(false)} title="Create New Course" size="lg">
        <div className="space-y-4">
          <div className="grid grid-cols-5 gap-3 mb-4">
            {[
              { id: 'tempo', from: 'from-tempo-600', to: 'to-tempo-700' },
                    ...
                    {gamificationLeaderboard.map((entry, i) => (
                      <tr key={entry.id} className={cn('border-b border-white/5', entry.id === currentEmployeeId && 'bg-tempo-500/5')}>
                        <td className="py-2.5 px-3">
                          {i === 0 ? <span className="text-yellow-500 font-bold text-sm">1</span> : i === 1 ? <span className="text-gray-400 font-bold text-sm">2</span> : i === 2 ? <span className="text-tempo-800 font-bold text-sm">3</span> : <span className="text-t3">{i + 1}</span>}
                        </td>
                        <td className="py-2.5 px-3">
                          <div className="flex items-center gap-2">
                      ...
                      <div className="flex items-center gap-3">
                        <div className="w-8 h-8 rounded-lg bg-tempo-500/10 flex items-center justify-center">
                          {source === 'course_complete' ? <CheckCircle size={14} className="text-green-400" /> :
                           ...
                           source === 'streak_bonus' ? <Activity size={14} className="text-tempo-400" /> :
                          ...
                          {blocks.sort((a: any, b: any) => a.order - b.order).map((block: any) => (
                            <div key={block.id} className="px-4 py-2 flex items-center gap-3 text-xs">
                              {block.type === 'video' ? <Video size={12} className="text-blue-500" /> :
                               ...
                               block.type === 'download' ? <Download size={12} className="text-tempo-700" /> :
