...
function SimplePieChart({ data, height = 240 }: { data: { label: string; value: number }[]; height?: number }) {
  ...
  const colors = ['var(--color-tempo-600)', '#3b82f6', '#10b981', '#8b5cf6', '#f59e0b', '#ef4444', '#06b6d4', '#64748b']
  let cumulative = 0
...
function SimpleLineChart({ data, height = 200 }: { data: { label: string; value: number }[]; height?: number }) {
  ...
  return (
    <svg viewBox={`0 0 ${w} ${height}`} className="w-full" preserveAspectRatio="xMidYMid meet">
      ...
      <path d={pathD} fill="none" stroke="var(--color-tempo-600)" strokeWidth={2} />
      {points.map((p, i) => (
        <circle key={i} cx={p.x} cy={p.y} r={3} fill="var(--color-tempo-600)" />
      ))}
      {data.map((d, i) => {
