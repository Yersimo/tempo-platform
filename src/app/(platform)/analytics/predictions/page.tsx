...
function AttritionTab({ data, getDeptName, departments }: { data: AttritionPrediction[]; getDeptName: any; departments: any[] }) {
  ...
  const riskColors: Record<string, string> = { critical: 'text-red-600 bg-red-50', high: 'text-tempo-700 bg-tempo-50', medium: 'text-amber-600 bg-amber-50', low: 'text-green-600 bg-green-50' }
      ...
      <Card>
        ...
        <LineChart
          ...
          series={[
            ...
            { label: 'Forecast', data: [...new Array(data.historical.length - 1).fill(NaN), historicalValues[historicalValues.length - 1], ...data.forecast.map(f => f.headcount)], color: 'var(--color-tempo-600)', dashed: true },
          ]}
      ...
      <Card>
        ...
        <LineChart
          ...
          series={[
            ...
            { label: 'Forecast', data: [...new Array(data.historical.length - 1).fill(NaN), historicalValues[historicalValues.length - 1], ...data.forecast.map(f => f.totalCost)], color: 'var(--color-tempo-600)', dashed: true },
          ]}
          confidence={{
            upper: [...new Array(data.historical.length).fill(historicalValues[historicalValues.length - 1] || 0), ...data.forecast.map(f => f.upper)],
            lower: [...new Array(data.historical.length).fill(historicalValues[historicalValues.length - 1] || 0), ...data.forecast.map(f => f.lower)],
      ...
      <Card>
        ...
        <LineChart
          ...
          series={[
            ...
            { label: 'Forecast', data: [...new Array(data.historical.length - 1).fill(NaN), data.historical[data.historical.length - 1]?.score ?? 70, ...data.forecast.map(f => f.score)], color: 'var(--color-tempo-600)', dashed: true },
          ]}
          confidence={{
