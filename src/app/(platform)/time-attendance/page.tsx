                ...
                <div className="text-center">
                  ...
                  {clockInTime && (
                    <p className="text-sm text-t3 mt-2">
                      ...
                      {onBreak && <span className="text-tempo-700 ml-2">(On Break)</span>}
                    </p>
                  )}
                </div>
            ...
            <Card className="lg:col-span-2">
              ...
              <div className="px-6 pb-6">
                      ...
                      <div key={entry.id} className="flex items-center justify-between p-3 rounded-lg border border-divider bg-canvas">
                        ...
                        <div className="flex items-center gap-3">
                          <div className="text-right">
                            ...
                            {(entry.overtime_hours || 0) > 0 && <p className="text-xs text-tempo-700">+{formatHours(entry.overtime_hours || 0)}h OT</p>}
                          </div>
                ...
                <div className="mt-6">
                  ...
                  {weeklyOvertime > 0 && (
                    <p className="text-xs text-tempo-700 mt-2 flex items-center gap-1">
                  ...
                  {timesheetData.size === 0 ? (
                    ...
                    return (
                      ...
                      <tr className="hover:bg-canvas/50 cursor-pointer" onClick={() => setExpandedTimesheetEmp(prev => prev === empId ? null : empId)}>
                        ...
                        {data.hours.map((h, i) => (
                          <td key={i} className={`px-3 py-3 text-center text-xs ${h > 8 ? 'text-tempo-700 font-semibold' : h > 0 ? 'text-t1' : 'text-t3'}`}>
                            {h > 0 ? formatHours(h) : '-'}
                          </td>
                        ))}
                        <td className="px-3 py-3 text-center text-xs font-bold text-t1">{formatHours(data.total)}</td>
                        <td className="px-3 py-3 text-center text-xs">
                          {data.overtime > 0 ? <span className="text-tempo-700 font-semibold">{formatHours(data.overtime)}</span> : '-'}
                        </td>
                              ...
                              {weekDates.map((date, i) => {
                                ...
                                return dayEntries.map(entry => (
                                  <div key={entry.id} className="flex items-center gap-4 p-2 rounded border border-divider bg-surface">
                                    ...
                                    {(entry.overtime_hours || 0) > 0 && <span className="text-xs text-tempo-700">+{formatHours(entry.overtime_hours || 0)} OT</span>}
                                    <StatusBadge status={entry.status} />
        ...
        <>
          ...
          <div className="grid grid-cols-7 gap-2 mb-6">
            {coverageByDay.map((day, i) => (
              <div key={day.date} className={`rounded-lg border p-3 text-center ${day.gap > 2 ? 'border-red-500/30 bg-red-500/5' : day.gap > 0 ? 'border-tempo-700/30 bg-tempo-700/5' : 'border-divider bg-canvas'}`}>
                ...
                {day.gap > 0 && (
                  <p className="text-xs text-tempo-700 mt-1 flex items-center justify-center gap-1">
                        ...
                        <td key={date} className="px-2 py-2 align-top border-r last:border-r-0 border-divider" style={{ minWidth: 140 }}>
                          <div className="space-y-1.5 min-h-[100px]">
                            {dayShifts.map(shift => {
                              ...
                              const colorClass = isUnassigned ? 'bg-tempo-50 border-tempo-200 text-tempo-800' :
                                ...
                                <div key={shift.id} className={`rounded-md border p-1.5 text-xs ${colorClass}`}>
                                  {isUnassigned ? (
                                    <>
                                      ...
                                      {role === 'employee' && (
                                        <button
                                          className="mt-1 w-full text-[10px] font-medium bg-tempo-700 text-white rounded px-1 py-0.5 hover:bg-tempo-700 transition-colors"
                ...
                {approachingOT.length === 0 ? (
                  ...
                  <div key={emp.id} className="flex items-center justify-between p-3 rounded-lg border border-divider">
                    ...
                    <div className="text-right">
                      ...
                      <p className={`text-xs ${remaining <= 2 ? 'text-red-500' : 'text-tempo-700'}`}>
                  ...
                  <tbody className="divide-y divide-border">
                    {pendingOTEntries.map(entry => (
                      <tr key={entry.id} className="hover:bg-canvas/50">
                        ...
                        <td className="px-4 py-3 text-sm text-center font-medium text-tempo-700">{formatHours(entry.overtime_hours || 0)}h</td>
                        ...
                        {timeOffPolicies.filter(p => p.is_active).map(policy => {
                          ...
                          return (
                            <td key={policy.id} className="px-3 py-3 text-center">
                              ...
                              {bal.pending > 0 && <p className="text-[10px] text-tempo-700 mt-0.5">{bal.pending}d pending</p>}
                            </td>
                          )
                        })}
                      </tr>
