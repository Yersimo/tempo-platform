  ...
  return (
    <>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
        ...
        <StatCard label={t('totalPairs')} value={mentoringPairs.length} change={tc('allTime')} changeType="neutral" />
      </div>
      ...
      {activeTab === 'programs' && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {mentoringPrograms.length === 0 ? (
            <Card>
              <div className="py-8 text-center text-sm text-t3">{t('noPrograms')}</div>
            </Card>
            ...
            <table className="w-full">
              ...
              <tbody className="divide-y divide-border">
                {mentoringPairs.length === 0 ? (
                  <tr><td colSpan={6} className="px-6 py-12 text-center text-sm text-t3">{t('noPairs')}</td></tr>
                    ...
                    <tr key={pair.id} className="hover:bg-canvas/50">
                      ...
                      <td className="px-4 py-3 text-center">
                        <div className="flex gap-1 justify-center">
                          {pair.status === 'active' && (
                            <>
                              <Button size="sm" variant="primary" onClick={() => completePair(pair.id)}>{tc('complete')}</Button>
                              <Button size="sm" variant="ghost" onClick={() => pausePair(pair.id)}>{tc('pause')}</Button>
                            </>
                          )}
                          {pair.status === 'paused' && (
      ...
      <Modal open={showProgramModal} onClose={() => setShowProgramModal(false)} title={t('createProgramModal')}>
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <Select label={t('programType')} value={programForm.type} onChange={(e) => setProgramForm({ ...programForm, type: e.target.value as 'one_on_one' | 'reverse' | 'group' })} options={[
