        ...
        <div className="cp-confetti-container">
          {Array.from({ length: 50 }).map((_, i) => (
            <div key={i} className="cp-confetti" style={{
              ...
              backgroundColor: ['var(--color-tempo-600)', '#3b82f6', '#22c55e', '#a855f7', '#eab308', '#ec4899'][i % 6],
            }} />
          ))}
        </div>
  ...
  function renderHeadingBlock(block: typeof blocks[0], status: BlockStatus) {
    return (
      <div className="course-player-prose">
        <h1 style={{ fontSize: '2rem', fontWeight: 800, marginBottom: '0.5rem', borderBottom: '3px solid var(--color-tempo-500, #285B7A)', paddingBottom: '0.75rem' }}>
    ...
    return (
      <div className="course-player-prose" style={{ textAlign: 'center', padding: '2rem 0' }}>
        <a
          ...
          style={{ display: 'inline-block', padding: '0.75rem 2rem', borderRadius: '8px', background: 'var(--color-tempo-600)', color: '#fff', fontWeight: 600, fontSize: '1rem', textDecoration: 'none', transition: 'opacity 0.2s' }}
